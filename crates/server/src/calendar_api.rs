...
#[derive(Debug, Serialize)]
pub(crate) struct CalendarSyncResponse {
    pub(crate) account_id: String,
    pub(crate) calendars_synced: usize,
    pub(crate) full_syncs: usize,
    pub(crate) incremental_syncs: usize,
    pub(crate) events_upserted: usize,
    pub(crate) events_cancelled: usize,
}
...
pub(crate) fn selected_sync_calendar_ids(account: &CalendarAccount) -> Vec<String> {
    account
...
pub(crate) async fn sync_selected_calendars(
    ...
    let adapter = state.calendar.google()?;
    let window = iana_date_range(start, end, account_timezone(state, account).await?.as_str())
        .map_err(ApiError::bad_request)?;
    let events = state.vault.external_event_repo();
    let cursors = state.vault.calendar_sync_cursor_repo();
    let now = OffsetDateTime::now_utc();
    ...
    for calendar_id in &selected_calendar_ids {
        let cursor = cursors
            .get(account.id.clone(), calendar_id.clone())
            .await
            .map_err(ApiError::internal)?;
        let (report, was_full) = if should_full_sync(force_full, cursor.as_ref(), now) {
                ...
                .await
            {
                ...
                Err(error) => return Err(map_calendar_error(error)),
            }
        };
...
#[derive(Debug, Serialize)]
pub(crate) struct CalendarWritebackResponse {
    pub(crate) account_id: String,
...
pub(crate) fn is_writeback_account(account: &CalendarAccount) -> bool {
    account.enabled
...
pub(crate) async fn writeback_schedule(
    ...
    let calendar_id = account
        ...
        .ok_or_else(|| ApiError::conflict("Create a managed calendar before write-back."))?;
    let adapter = state.calendar.google()?;
    ...
    let links_repo = state.vault.managed_calendar_event_link_repo();
    ...
    let now = OffsetDateTime::now_utc();
    for block in &blocks {
        let update_draft = mnema_infra::calendar::CalendarEventDraft {
        ...
        let create_draft = mnema_infra::calendar::CalendarEventDraft {
        ...
        link.updated_at = now;
        links_repo.upsert(link).await.map_err(ApiError::internal)?;
    }
    ...
    for link in links_repo
        .list_for_account(account.id.clone())
        .await
        .map_err(ApiError::internal)?
    {
        ...
        let block = schedule_repo
            .find(link.schedule_block_id.clone())
            .await
            .map_err(ApiError::internal)?;
        let should_delete = block.as_ref().is_none_or(|block| {
            (!is_writeback_block(block)
...
fn managed_provider_event_id(block_id: &ScheduleBlockId) -> String {
    format!("mnema{}", block_id.0.to_string().replace('-', "")).to_ascii_lowercase()
}

fn is_writeback_block(block: &ScheduleBlock) -> bool {
    matches!(
        block.source,
